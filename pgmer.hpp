...
public:
    ...
    Pgmer(const std::string &name, int row, int col, int *data) : name(name), row(row), col(col), data(data) {}
    ...
    bool ReadImg(std::string file_name) {
        ...
        this->data = new int[(nr * nc * sizeof(int))];
        if (this->data == NULL)
        {
    ...
    bool SetPix(int i, int j, int val) {
